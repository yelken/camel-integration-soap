...
= Installing the Integreatly lab environment - for Instructors
...
The purpose of this installation guide is to utilize a set of Ansible playbooks that can be used to install a range of Red Hat middleware products on OpenShift. These middleware products on OpenShift form the lab environment for this course.
...
The following command installs Integreatly using both the GitHub client ID and the GitHub client secret.
...
[TIP]
====
The following command installs Integreatly using an existing OPENTLC OCP Cluster private key:

[source,shell]
----
$ ansible-playbook -i inventories/hosts playbooks/install.yml -e github_client_id=<your_client-id> -e github_client_secret=<your_client_secret> -e ansible_ssh_private_key_file=~/.ssh/<your_ssh_private-key>.pem
----

To locate the *<your_ssh_private-key>.pem* file, use this command first:

[source,shell]
----
$ ls ~/.ssh/<your_ssh_private-key>.pem
----
