...
:toc2:
:linkattrs:
:ignite_template: link:https://raw.githubusercontent.com/syndesisio/syndesis/master/install/syndesis.yml[Fuse Online template]
...
== Fuse Online Provisioning Lab

.Goal
* Provision Fuse Online in an OpenShift environment
...
* As a component of Red Hat Fuse called _Fuse Online_ that is deployed in an OpenShift Container Platform environment.
** The focus of this lab is on this approach.

Fuse Online is provisioned in a single remote OpenShift project. For the purposes of this lab, this single remote OpenShift project is provided to you and does not require you to install OpenShift Container Platform.

== Install `oc` Utility

The majority of the Fuse Online installation procedures described in this lab use the OpenShift Container Platform web console. There is one step in the process that needs to be executed at the command line using the OpenShift `oc` utility.
...
== Delete Fuse Online Environment

The Fuse Online environment that was provisioned for earlier labs is no longer needed and will be decommissioned.
...
. In the resulting *Services* page, expand the *All Services* tab to reveal the VM name for the Fuse Online environment.
+
NOTE: The name of the VM should resemble `OTLC-LAB-hchin-redhat.com-PROD_SHARED_WORKLOAD_FUSE_IGNITE-5ffe`
+
. Select the VM.
. Click the *App Control* button at the top menu bar.
. In the drop-down box that appears, click *Delete*.
+
image::images/vm_delete_fuse_found_part1.png[]
+
. Once the page loads, navigate to the *Are you sure?* section and select the *Yes* radio button.
. Click *Submit*.
+
image::images/vm_delete_app_status.png[]
+
. Check your email inbox for a message regarding the deletion of the Fuse Online VM.

After you receive the email notification, proceed to the next section.
...
.. *Display Name*: `Manually Provisioned Fuse Online`
.. *Description*: `Manually Provisioned Fuse Online`
. Click *Create*.
...
** The assigned `clusterquota` and `limitrange` allow for a successful deployment of Fuse Online.
...
Now that you have access to an OpenShift Container Platform lab environment and have created a project, you are prepared to provision Fuse Online in that project.

== Review Fuse Online Artifacts

Fuse Online includes an OpenShift template that defines most of the artifacts to be provisioned for a Fuse Online deployment. Generally, the templates are intended to be tweaked as needed by application developers. However, for the purposes of this lab, the {ignite_template} is sufficient without changes.
...
** `syndesis-ui`: The Web application that provides the Fuse Online console interface
...
** `syndesis-oauthproxy`: Interacts with the OAuth2 provider implementation of OpenShift to proxy authentication and authorization needs for the Fuse Online web application
...
.. How many persistent volumes are needed by a Fuse Online deployment?
...
** The `serviceaccounts.openshift.io/oauth-redirecturi.local` annotation specifies the static URL for the Fuse Online console interface (`syndesis-ui`)
...
== Provision Fuse Online

. In the OpenShift Container Platform web console, navigate to the *Overview* screen of your *Manually Provisioned Fuse Online* project.
...
-----
* Later, you copy and paste this long token string as a parameter of the Fuse Online template.

. Import the Fuse Online template:
...
. Return to the *Overview* screen of your *Manually Provisioned Fuse Online* project.
...
* Expect to see that your Fuse Online deployment is well within the limits defined by `clusterquota`:
+
...
* Once you log in, expect to see the Fuse Online *Home* screen:
+
image::images/fi_home.png[]

You have successfully provisioned Fuse Online in an OpenShift Container Platform environment.
